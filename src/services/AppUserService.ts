...
export class AppUserService {
  ...
  // Delete methods matching .NET reference
  static async deleteAppUserById(userId: string): Promise<boolean> {
    const user = await UserRepository.findAppUserById(userId);
    if (!user) {
      throw new CustomException('User does not exist');
    }

    await UserRepository.deleteAppUserById(userId);
    return true;
  }

  static async deleteAppUserByEmail(email: string, token?: string): Promise<boolean> {
    const user = await UserRepository.findAppUserByEmail(email.trim().toLowerCase());
    if (!user) {
      throw new CustomException('User does not exist');
    }

    await UserRepository.deleteAppUserByEmail(email.trim().toLowerCase());
    return true;
  }
