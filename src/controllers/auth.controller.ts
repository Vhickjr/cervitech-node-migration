...
export const AuthController = {
  ...
  async authenticate(req: Request, res: Response) {
    try {
      ...
      if (!result.success) {
        logger.warn("Authentication failed", { emailOrUsername: body.emailOrUsername, message: result.message });
        return res.status(400).json({ success: false, message: result.message });
      }
  ...
  async resetPassword(req: Request, res: Response) {
    try {
      ...
      const newPassword = req.body.NewPassword ?? req.body.newPassword ?? req.body.ConfirmNewPassword;
