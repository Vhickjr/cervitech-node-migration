 ...
 * failed (with the error), or still awaiting confirmation. DeviceNotRegistered
 * tokens are cleared so they stop being retried, and any confirmed failures
...
export async function runPushReceiptPolling(now: Date = new Date()): Promise<number> {
  ...
  const receipts = await PushNotificationDriver.getReceipts(ticketIds);
  ...
  if (failedCount > 0) {
    await notifyPushFailure(
      `Receipts: ${failedCount} push(es) not delivered (${failures.join('; ')})`
    );
  }
