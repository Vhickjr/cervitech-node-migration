...
import EToken from '../models/token';
...
export class AuthService {
  ...
  static async sendResetToken(email: string): Promise<void> {
  // Check if user exists
  const user = await User.findOne({ email }); // adjust to your ORM
  if (!user) {
  ...
  // Store token (example using MongoDB schema)
  await EToken.create({
    email,
    token: eToken,
    expiresAt,
  ...
  // Send email
  await sendPasswordResetEmail(email, eToken);
}
}
